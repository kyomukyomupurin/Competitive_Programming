...
class Timer{
 public:
  Timer(std::chrono::system_clock::time_point start = std::chrono::system_clock::now(),
        std::chrono::system_clock::time_point stop = std::chrono::system_clock::now()) : start_(start), stop_(stop) {}
  void Start();
  void Stop();
